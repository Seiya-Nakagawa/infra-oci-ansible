...
- **暗号化強化**: 脆弱な暗号スイート・MAC・KEXアルゴリズムを無効化し、強力なもの (例: `aes256-gcm@openssh.com`, `chacha20-poly1305@openssh.com` 等) のみ許可。
  - **説明**: 旧式の脆弱なアルゴリズムを排除し、現代の暗号技術に基づいたセキュアな通信を強制する。
...
- **データ永続性**:
  ...
  - 今後、一般利用者の増加や本格的なサービス運用が必要となった段階で、マネージドなデータベース（OCI MySQL/PostgreSQL等）の導入や、ボリューム・バックアップ・ポリシーの適用など、データ保護・永続化の強化を改めて検討する。
